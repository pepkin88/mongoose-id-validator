                ...
                validator: function (value, respond) {
                    ...
                    if (!(self instanceof IdValidator) || self.enabled) {
                        ...
                        try {
                            ...
                            return respond(!!(options && options.isPermissive))
                        }
                    }
                    return respond(true)
                },
...
function executeQuery (query, conditions, validateValue, respond) {
    query.and([conditions]).exec(function (err, count) {
        if (err) {
            return respond(err)
        }
...
function validateId (
    ...
    var query = refModel.countDocuments ? refModel.countDocuments() : refModel.count()
...
function validateIdArray (
    ...
    var refModel = connection.model(refModelName)
    var query = refModel.countDocuments ? refModel.countDocuments() : refModel.count()
    query.where('_id')['in'](checkValues)
